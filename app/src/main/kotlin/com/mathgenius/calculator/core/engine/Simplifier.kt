...
// Enhanced Simplifier with Better Constant Folding
...
class Simplifier {
    ...
    private fun simplifyMultiplication(left: Expr, right: Expr): Expr {
        // 提取所有常数并相乘
        val (constants, nonConstants) = extractMultiplicationFactors(Expr.Binary(left, BinaryOp.MULTIPLY, right))

        // 计算所有常数的乘积
        val constantProduct = constants.fold(1.0) { acc, c -> acc * c.value }

        // 特殊情况处理
        when {
            // 如果乘积为 0,整个表达式为 0
            constantProduct == 0.0 -> return Expr.Constant(0.0)

            // 如果没有非常数因子
            nonConstants.isEmpty() -> return Expr.Constant(constantProduct)

            // 如果常数乘积为 1,省略它
            constantProduct == 1.0 -> {
                return if (nonConstants.size == 1) {
                    nonConstants[0]
                } else {
                    buildMultiplicationChain(nonConstants)
                }
            }

            // 如果常数乘积为 -1,提取负号
            constantProduct == -1.0 -> {
                val expr = if (nonConstants.size == 1) {
                    nonConstants[0]
                } else {
                    buildMultiplicationChain(nonConstants)
                }
                return Expr.Unary(UnaryOp.NEGATE, expr)
            }

            // 一般情况: 常数 * 其他因子
            else -> {
                val allFactors = mutableListOf<Expr>(Expr.Constant(constantProduct))
                allFactors.addAll(nonConstants)
                return buildMultiplicationChain(allFactors)
            }
        }
    }
     ...
     * 返回: (常数列表, 非常数列表)
     */
    private fun extractMultiplicationFactors(expr: Expr): Pair<List<Expr.Constant>, List<Expr>> {
        val constants = mutableListOf<Expr.Constant>()
        val nonConstants = mutableListOf<Expr>()

        fun extract(e: Expr) {
            when (e) {
                is Expr.Constant -> constants.add(e)
                is Expr.Binary -> {
                ...
                else -> nonConstants.add(e)
            }
        }

        extract(expr)
        return Pair(constants, nonConstants)
    }

    /**
     * 构建乘法链表达式
     */
    private fun buildMultiplicationChain(factors: List<Expr>): Expr {
        return factors.reduce { acc, factor ->
            Expr.Binary(acc, BinaryOp.MULTIPLY, factor)
        }
    }
    ...
    private fun expandPower(expr: Expr.Binary): Expr {
        ...
        // 只展开小整数幂
        if (exponent is Expr.Constant &&
            exponent.value > 0 &&
            exponent.value <= 5 &&
            exponent.value == exponent.value.toInt().toDouble()) {
